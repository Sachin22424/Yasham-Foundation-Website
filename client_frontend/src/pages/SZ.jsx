import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { Container, Row, Col, Carousel, Card } from 'react-bootstrap';
import 'bootstrap/dist/css/bootstrap.min.css';
import '../assets/YCM.css';

const SZ = () => {
    const [szData, setSzData] = useState(null);

    useEffect(() => {
        const fetchSzData = async () => {
            try {
                const response = await axios.get('https://yasham-foundation-website-production.up.railway.app/api/sz');
                setSzData(response.data[0]); // Assuming the API returns an array and we need the first item
            } catch (error) {
                console.error('Error fetching SZ data:', error);
            }
        };

        fetchSzData();
    }, []);

    if (!szData) {
        return <div>Loading...</div>;
    }

    return (
        <>
            <Col>
                <Carousel>
                    {szData.images.map((image, index) => (
                        <Carousel.Item key={index} interval={3000}>
                            <img
                                className="d-block w-100 carousel-img"
                                src={image.url}
                                alt={`Slide ${index + 1}`}
                            />
                            <Carousel.Caption>
                                <h3>{image.caption}</h3>
                            </Carousel.Caption>
                        </Carousel.Item>
                    ))}
                </Carousel>
            </Col>
            <Container className="my-2">
                <Card className="p-3 mb-4" style={{ border: 'none', backgroundColor: '#f9f9f9' }}>
                    <Card.Body>
                        <Card.Text className="text-center" style={{ color: "#333333", fontSize: "1.2em", lineHeight: "1.6" }}>
                            <h1 style={{ fontWeight: '700', }}>{szData.title}</h1>
                        </Card.Text>
                        <Card.Text style={{ color: "#333333", fontSize: "1.2em", lineHeight: "1.6" }}>
                            {szData.description}
                        </Card.Text>
                    </Card.Body>
                </Card>
            </Container>
        </>
    );
};

export default SZ;